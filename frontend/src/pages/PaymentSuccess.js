  ...
  if (status === "success" && report) {
    return (
      <div className="min-h-screen bg-[#05050A] text-white">
        <div className="pt-12 text-center"><CheckCircle className="w-14 h-14 text-[#39FF14] mx-auto mb-3" /><h1 className="text-2xl font-bold">Pago Test confirmado</h1></div>
  ...
  return (
    <div className="min-h-screen bg-[#05050A] flex items-center justify-center text-white px-4" data-testid="payment-success-page">
      <div className="text-center max-w-md">
        {status === "processing" ? (
          <><Loader2 className="w-12 h-12 text-[#00E5FF] animate-spin mx-auto mb-4" /><p className="text-lg text-zinc-300">Confirmando el pago Test con Stripe…</p></>
        ) : (
          <><XCircle className="w-16 h-16 text-[#FF3B30] mx-auto mb-4" /><p className="text-lg text-red-400 mb-4">No se ha podido confirmar un pago Test válido.</p><button onClick={() => navigate("/")} className="px-6 py-3 rounded-full bg-white/10 text-white hover:bg-white/20">Volver</button></>
        )}
      </div>
    </div>
  );
